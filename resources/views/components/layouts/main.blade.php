    ...
    <div class="flex w-full max-w-7xl h-[92vh] rounded-3xl overflow-hidden shadow-2xl">
        ...
        <div class="w-20 bg-slate-900 flex flex-col items-center py-6 gap-3 flex-shrink-0">
            ...
            @if(in_array(auth()->user()->role, ['kasir', 'superadmin']))
                <a href="{{ route('kasir.dashboard') }}" class="sidebar-icon w-12 h-12 flex items-center justify-center rounded-xl {{ request()->routeIs('kasir.dashboard') ? 'nav-active' : 'hover:bg-slate-700' }}">
                    <svg class="w-6 h-6 {{ request()->routeIs('kasir.dashboard') ? 'text-white' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
